...
class PalindromeAppTest < Minitest::Test
  ...
  def test_about
    ...
    assert last_response.ok?
    assert doc(last_response).at_css('h1')
    assert_equal "#{@base_title} | About", doc(last_response).at_css('title').content
  end
  ...
  def test_palindrome
    ...
    assert last_response.ok?
    assert doc(last_response).at_css('h1')
    assert_equal "#{@base_title} | Palindrome Detector", doc(last_response).at_css('title').content
  end
end
